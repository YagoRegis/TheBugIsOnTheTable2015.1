...
sub turn(int angle, int power, int direction ) {
	...
	int car_arc = ( angle * PI * CAR_RADIUS ) /
		HALF_ROTATION;

	// Measured in centimeter
	int amount_turns = (HALF_ROTATION * car_arc) /
		(PI * WHEEL_RADIUS);

	RotateMotorExPID(OUT_BC, power, amount_turns,
		100 * direction, true, true, 40, 40, 90);
}

sub move(int distance, int power, int direction) {
	int angle = (distance * COMPLETE_ROTATION * direction) /
		(PI * WHEEL_DIAMETER);
